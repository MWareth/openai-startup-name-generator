// Computes how much of a target an agent has achieved within the target's
// period, plus tier progress. Targets are measured in GROSS COMMISSION.
export async function getTargetProgress(supabase, target) {
  let q = supabase
    .from('deals')
    .select('deal_value, gross_commission, agent_commission, closed_on')
  ...
  // Progress is measured against GROSS COMMISSION generated.
  const achieved = (deals || []).reduce((s, d) => s + Number(d.gross_commission || 0), 0);
  const salesValue = (deals || []).reduce((s, d) => s + Number(d.deal_value || 0), 0);
