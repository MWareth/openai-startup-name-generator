...
```bash
# Look in the project (Claude Code on the web) first, then the personal install
...
```bash
# Use whichever location the script was found in (project path preferred)
SCRIPT=".claude/skills/realestate/scripts/generate_realestate_pdf.py"
[ -f "$SCRIPT" ] || SCRIPT="$HOME/.claude/skills/realestate/scripts/generate_realestate_pdf.py"
python3 "$SCRIPT"
```
