---
name: realestate
title: AI Real Estate Analyst — Main Orchestrator
description: Entry point for the AI Real Estate Analyst toolkit. Use when the user wants to analyze a property, run comps, estimate rental income or cash flow, evaluate an investment (buy-hold, BRRRR, flip), screen properties, compare addresses, write an MLS listing, run a mortgage/affordability calculation, assess a neighborhood or local market, analyze commercial property, or generate a client-ready PDF property report — typically via /realestate <command> <address>.
---
